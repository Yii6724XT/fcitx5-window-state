...
// ---------------------------------------------------------------------------
// Fcitx5 D-Bus interface (verified on 2026-06-10, Fcitx 5.1.19)
//
...
const DEFAULT_IM = "keyboard-us"; // US English
...
export default class Fcitx5WindowStateExtension extends Extension {
  ...
  disable() {
    // 1. Display signals
    for (const id of this._signalIds) global.display.disconnect(id);
    this._signalIds = [];
  ...
  _onFcitx5Appeared() {
    ...
    try {
      ...
      if (
        ...
        this._dbg(
          `applying cached IM "${cached}" for focused win=${win.get_id()}`,
        );
  ...
  // ======================================================================
  _pollCurrentIM() {
    if (!this._proxy || this._pollInFlight) return;
    ...
    this._proxy.CurrentInputMethodRemote((result, error) => {
      ...
      if (current === this._lastKnownIM) return; // no change

      this._dbg(
        `poll DETECTED change: "${this._lastKnownIM}" -> "${current}"${this._inProgrammaticSwitch ? " (IGNORED: inProgrammaticSwitch)" : ""}`,
      );

      this._lastKnownIM = current;

      // Ignore changes that we ourselves triggered via SetCurrentIM()
      if (this._inProgrammaticSwitch) return;

      // Feature B: Persist the new IM for the focused window
      const win = global.display.focus_window;
      if (!win || win.get_window_type() !== Meta.WindowType.NORMAL) {
        this._dbg(
          `poll save SKIP: no focused NORMAL window (type=${win ? win.get_window_type() : "null"})`,
        );
        return;
      }
      ...
      this._dbg(
        `poll SAVED win=${win.get_id()} wmClass="${wmClass}" "${prev}" -> "${current}" (stateMap.size=${this._stateMap.size})`,
      );
    });
  }
  ...
  _onFocusWindowChanged() {
    ...
    if (this._isLauncher(wmClass)) {
      this._dbg(
        `FOCUS: win=${winId} wmClass="${wmClass}" LAUNCHER -> force "${DEFAULT_IM}"`,
      );
    ...
    if (parent && this._stateMap.has(parent.get_id())) {
      ...
      this._dbg(
        `FOCUS: win=${winId} wmClass="${wmClass}" TRANSIENT of ${parent.get_id()} inherit "${parentIM}"`,
      );
    ...
    if (this._stateMap.has(winId)) {
      this._dbg(
        `FOCUS: win=${winId} wmClass="${wmClass}" RESTORE cached="${cached}" (stateMap.size=${this._stateMap.size})`,
      );
      this._setCurrentIM(this._stateMap.get(winId));
    } else {
      this._dbg(
        `FOCUS: win=${winId} wmClass="${wmClass}" NEW -> init to "${DEFAULT_IM}" (stateMap.size=${this._stateMap.size})`,
      );
  ...
  _onOverviewShowing() {
    ...
    this._dbg(
      `OVERVIEW show saveWinId=${this._overviewWindowId} force "${DEFAULT_IM}"`,
    );
    this._setCurrentIM(DEFAULT_IM);
  }
  ...
  _onOverviewHiding() {
    ...
    const focusWinId =
      focusWin && focusWin.get_window_type() === Meta.WindowType.NORMAL
        ? focusWin.get_id()
        : null;
    ...
    if (focusWinId !== null && focusWinId !== this._overviewWindowId) {
      this._dbg(
        `OVERVIEW hide SKIP (focus changed: ${this._overviewWindowId} -> ${focusWinId})`,
      );
      this._overviewWindowId = null;
      return;
    }
    ...
    const restore =
      ...
      this._stateMap.has(this._overviewWindowId)
        ? this._stateMap.get(this._overviewWindowId)
        : null;
    this._dbg(
      `OVERVIEW hide prevWinId=${this._overviewWindowId} restore="${restore}"`,
    );
    if (
      this._overviewWindowId !== null &&
      this._stateMap.has(this._overviewWindowId)
  ...
  // ======================================================================
  _trackWindow(win) {
    if (this._windowSignalMap.has(win)) return;
    ...
    this._dbg(
      `TRACK win=${winId} (trackedWindows=${this._windowSignalMap.size})`,
    );
  }
  ...
  _setCurrentIM(imName) {
    ...
    this._dbg(
      `setIM SCHEDULE "${im}"${hadPending ? " (cancelled previous)" : ""} debounce=${DEBOUNCE_MS}ms`,
    );
    ...
    this._debounceSource = GLib.timeout_add(
      ...
      () => {
        ...
        if (!this._proxy || !this._pendingIm) {
          this._dbg(
            `setIM FIRE SKIP: proxy=${!!this._proxy} pending="${this._pendingIm}"`,
          );
        ...
        this._proxy.SetCurrentIMRemote(target, (result, error) => {
          if (error) {
            ...
            this._dbg(
              `setIM OK "${target}" lastKnownIM="${this._lastKnownIM}"`,
            );
          }
          this._inProgrammaticSwitch = false;
        });
